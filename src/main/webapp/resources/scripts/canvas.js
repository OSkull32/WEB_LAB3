...
window.addEventListener("DOMContentLoaded", () => {
    ...
    canvasPlot.addEventListener("click", function (event) {
        ...
        let r = document.querySelector("#r-select").value;
        ...
        if (tableX < -4 || tableX > 4) {
            ...
            if (tableY < -5 || tableY > 5) {
                ...
                sendData(tableX.toFixed(2), tableY.toFixed(2), r);
            }
        }
    });
...
function drawRect(r) {
    ...
    ctx.rect(xAxis, yAxis, scaleX * r, -scaleY * r / 2);
    ctx.closePath();
...
function drawArc(r) {
    ...
    ctx.arc(xAxis, yAxis, scaleX * (r / 2), -Math.PI , -Math.PI / 2, false);
    ctx.closePath();
    ctx.strokeStyle = "#ffba08";
    ctx.fillStyle = "rgba(163, 155, 168, 0.5)";
    ctx.fill();
...
function drawTriangle(r) {
    ctx.beginPath();
    ctx.moveTo(xAxis, yAxis);
    ctx.lineTo(xAxis, yAxis + scaleX * (r));
